...
if (!$product['hide_product']) {
	...
	echo '<div class="foxyshop_product_image">';
		if ( $thumbnailSRC = foxyshop_get_main_image("medium") ) {
			$thumb = aq_resize( $thumbnailSRC, 263, 170, 1, false );
			echo '<a href="' . $product['url'] . '"><img src="' . $thumbnailSRC . '" alt="' . htmlspecialchars($product['name']) . '" class="foxyshop_main_image" /></a>';
		}
	echo "</div>\n";
